...
//	return (current.xCoord == -13215 && current.levelID == 1 && current.levelTime > 0 && old.levelTime < 0.05);
	return (old.xCoord == -13213 && current.xCoord != -13213 && current.levelID == 1); // Hack to zero the timer due to using current rules Starting point not necessary with a proper start.
}
...
if (current.levelID == 1) {
	...
	return ((current.levelID > old.levelID && settings["split0"]) || (current.levelID == 6 && current.mirrorCounter == 6 && old.mirrorCounter != 6));
}
